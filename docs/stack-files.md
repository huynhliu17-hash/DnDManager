...
bot/                    — Discord bot (separate Node process, own package.json)
  ...
  lib/
    ...
    links.js            — readLinks / getWebAppUserId / setLink / getActiveSheetId / setActiveSheetId / verifyCredentials
  ...
  commands/
    ...
    dndcommands.js      — /dndcommands [all]: list all bot commands; ephemeral by default, public with all:True; paginated (dc: customIdPrefix)
  scripts/
    seed-dnd-index.js   — fetches 5e API name lists → data/dnd5e-index.json (run once)
  lib/
    dnd5e.js            — fuzzySearch / fetchDetail for 5e API lookups
  data/
    links.json          — {discordUserId → webAppUserId} (runtime, gitignored)
    active_sheets.json  — {discordUserId → characterSheetId} (runtime, gitignored)
    dnd5e-index.json    — cached 5e name index (gitignored, regenerated by seed script)
